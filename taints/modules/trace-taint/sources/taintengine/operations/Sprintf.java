...
public class Sprintf extends Operation {
    private final Set<Character> specifier = new HashSet<>(Arrays.asList('c', 'd', 'i', 'e', 'E', 'f', 'g', 'G', 'o', 's', 'u', 'x', 'X', 'p', 'n', '%'));
    ...
    public void handleArrayAccess(NodeMapper nodeMapper, EventSender eventSender, ArrayIndexMapper arMapper) {
        ...
        // TODO if the allocated memory was larger as the string that is stored, then we cut here the size of
    ...
    protected char[] getFormat() {
        String rawFormat;
        if (getOperands().length > 5) {
            // in this case we are parsing a "*sn" function, thus the format is at position 3
            // and the operands start one position later
             rawFormat = getOperands()[3].getValue();
             operandCounter++;
        } else {
            rawFormat = getOperands()[2].getValue();
        }
