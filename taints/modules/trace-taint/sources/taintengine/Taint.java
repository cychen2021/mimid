...
public final class Taint implements Iterable<HashMap<Long, BitSet>> {
    ...
    private final List<HashMap<Long, BitSet>> taintedBytes = new ArrayList<>(8);
    ...
    public boolean hasOnlyTaintType(TaintType taintType) {
        return taintTypes.contains(taintType) && 1 == taintTypes.size();
    }
    ...
    @Override
    public int hashCode() {
        return taintedBytes.hashCode() * 31 + taintTypes.hashCode();
    }
    ...
    public int getMaxTaint() {
        int maxBit = -1;
        for (var tnt : this.taintedBytes.get(0).values()) {
            maxBit = Math.max(tnt.length(), maxBit);
        }
        return maxBit;
    }
}
