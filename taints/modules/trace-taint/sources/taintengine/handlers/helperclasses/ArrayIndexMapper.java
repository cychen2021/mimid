...
public class ArrayIndexMapper {
    ...
    private final Deque<Map<String, Long>> variableIndexMap = new LinkedList<>();

    // maps variable to name of accessed array
    private final Deque<Map<String, String>> variableAccessMap = new LinkedList<>();

    // maps the index of an array to the size of an array
    private final Map<Long, Integer> indexSizeMap = new HashMap<>(100);

    // maps the variable name to the size of the allocated memory. This map is only a temporary storage.
    private final Deque<Map<String, Integer>> nameSizeMap = new LinkedList<>();
    ...
    public ArrayIndexMapper(ProgramInputInformation[][] argvInformation) {
        Map<String, Long> newMap = new HashMap<>(20);
        variableIndexMap.addLast(newMap);

        Map<String, String> newAccessMap = new HashMap<>(20);
        variableAccessMap.addLast(newAccessMap);

        Map<String, Integer> newSizeMap = new HashMap<>(20);
    ...
    public void methodCall() {
        variableIndexMap.add(new HashMap<>(5));

        variableAccessMap.addLast(new HashMap<>(5));

        nameSizeMap.addLast(new HashMap<>(5));
    }
