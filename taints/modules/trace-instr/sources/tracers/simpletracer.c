...
int tracerllvm_isTaintFile(int fd) {
    return fd == fileno(taintAnalysisCallFile);
}
...
// for libc wrapped functions we need to add function specific information in some cases
void tracerllvm_addInformation(int opcode)
{
    if (!traceStarted) { return; }
    if (noInstrument) { return; }
    ...
    tracerllvm_callInstruction("addInfo", "", "", "");
