...
//        additionalOperators["getc"]            = new foperator(FGETC,        true,  true);
//        additionalOperators["fread"]           = new foperator(TRACERFREAD,  true,  true);
...
//        additionalOperators["strtold"]         = new foperator(STRTO,        false, true);
        additionalOperators["strtok"]          = new foperator(STRTOK,       false, true);
        additionalOperators["strstr"]          = new foperator(STRSTR,       false, true);
        ...
        additionalOperators["\01_strtok"]      = new foperator(STRTOK,       false, true);
        additionalOperators["\01_strstr"]      = new foperator(STRSTR,       false, true);
        ...
        additionalOperators["__strdup"]       = new foperator(STRDUP,        false, true);
        additionalOperators["strdup"]         = new foperator(STRDUP,        false, true);
        ...
        additionalOperators["snprintf"]         = new foperator(SPRINTF,      false, true);
        ...
        additionalOperators["cmimid__"]        = new foperator(CMIMID,       false, true);
//        additionalOperators["method__exit"]    = new foperator(CMIMID,       false, false);
//        additionalOperators["stack__enter"]    = new foperator(CMIMID,       false, false);
//        additionalOperators["stack__exit"]     = new foperator(CMIMID,       false, false);
//        additionalOperators["scope__enter"]    = new foperator(CMIMID,       false, false);
//        additionalOperators["scope__exit"]     = new foperator(CMIMID,       false, false);
    }
...
//        }
     ...
     */
    void createFileHandlingCalls(IRBuilder<>* builder, Instruction* instr, const std::string& funNameString, bool printStrings, unsigned int opcode)
    {
        Value* funName = getGlobalString(builder, instr->getFunction()->getName());
        Value* retName = getGlobalString(builder, instr->getName());
        Value* retType = getGlobalString(builder, toString(instr->getType()));
        ...
        builderMutex.lock();
        auto builder_opcode = builder->getInt64(opcode);
        builder->CreateCall(tracer("instructionHeader"), {builder_opcode, funName, retName, retType});
        builderMutex.unlock();
    ...
    {
        auto funOperator = additionalOperators.find("cmimid__");
        if (funNameString.find("cmimid__") != 0) {
            funOperator = additionalOperators.find(funNameString);
        ...
        createFileHandlingCalls(nextInstructionBuilder, instr, funNameString, funOperator->second->use_string, funOperator->second->opcode);
        return true;
    }
...
struct TaintTrackPlugin : public ModulePass
{
    ...
    bool runOnModule(Module& M)
    {
        ...
        for (Function& f : M.functions())
        {
            ...
            if (f.getName().startswith("tracerllvm_") || f.getName().startswith("cmimid__"))
            {
                continue;
            }
