...
class ParsingStageExtractor:
    ...
    @classmethod
    def has_parsing_functions(cls):
        return any(el == Stage.PARSING for el in cls.stages.values())
        ...
        for obj in objs:
            if obj["type"] == "INPUT_COMPARISON":
                ...
                if function not in cls.stages:
                    ...
                    elif obj["operator"] not in ["tokenstore", "eof", "conversion"]:
    ...
    def _get_stage_mapping(cls, tmp_stages, current, seen, stage):
        ...
        if current in tmp_stages:
            if current == "_real_program_main":
                cls.stages[current] = Stage.UNDEF
            ...
            elif tmp_stages[current][Stage.LEXING] > tmp_stages[current][Stage.PARSING] or stage == Stage.LEXING:
            # elif tmp_stages[current][Stage.LEXING] > 0 or stage == Stage.LEXING:
                cls.stages[current] = Stage.LEXING
                current_stage = Stage.LEXING
            elif tmp_stages[current][Stage.PARSING] == 0:
                cls.stages[current] = Stage.UNDEF
                current_stage = Stage.UNDEF
            else:
