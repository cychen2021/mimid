...
import core.ComparisonCoverer as CompCov
...
def _add_values_to_queue(h_value, inpt_counter, new_covered, values, parent_input: "InputWrapper", re_evaluation: bool = False):
    ...
    if h_value.cover_counter[0] >= 0:
        for val in values:
            ...
            new_h_value.set_input_counter_adapt_value(0)
            ...
            if val.operator != "tokencomp":
                adjust_non_token_value(h_value, new_h_value, val)
            else:
                new_h_value.adjust_value(0)# + token_usage_adjustement)
...
def adjust_non_token_value(h_value, new_h_value, val):
    interesting_comparison_adjustement = CompCov.is_interesting_comparison(val.id, val.correction, val.at, h_value.cover_counter[0] == 0)
    new_h_value.adjust_value(len(val.correction) * 2)

    if interesting_comparison_adjustement > 0:
        new_h_value.adjust_value(interesting_comparison_adjustement)
    # try to close inputs that already achieve some coverage by using comparisons first that are lower on the stack
    elif h_value.cover_counter[0] > 0:
        new_h_value.adjust_value(100 - val.stack_size)
    elif h_value.cover_counter[0] == 0:
        h_value_decrement = 0
        # discourage inputs with same prefix as valid inputs only if also no promising comparison can be done
        for valid_found in Utils.valid_found:
            for i in range(min(len(valid_found), len(val.inp))):
                if valid_found[i] == val.inp[i]:
                    h_value_decrement += 1
                else:
                    break
        new_h_value.adjust_value(-h_value_decrement)
