...
def run_loop(parentdir, arg: bool, subject: str):
    ...
    for key, val in metric.count_regex(os.path.basename(subject), [el[0] for el in for_metric]).items():
        if val > 0:
            print("\n\t%s: %s" % (key, val))
    # os.system("grep '#####' *.gcov")
...
if __name__ == '__main__':
    ...
    # instrument program
    os.system("gcc %s -o %s.orig -ldl -lm" % (subject, subject))
    os.system("gcc -fprofile-arcs -ftest-coverage %s -o %s.cov -ldl -lm -DCOVERAGE" % (subject, subject))
